...
for pycache_path in pycache_paths:
    if os.path.exists(pycache_path):
        ...
        xbmc.log(f"{pycache_path} was deleted.", level=xbmc.LOGINFO)  # Protokollierung des Löschens
    else:
        xbmc.log(f"{pycache_path} does not exist.", level=xbmc.LOGINFO)  # Protokollierung der Nichtexistenz

# Anzeigen einer Benachrichtigung
dialog = xbmcgui.Dialog()
dialog.notification('TV Series NFO', 'Cache cleared.', xbmcgui.NOTIFICATION_INFO, 5000)
